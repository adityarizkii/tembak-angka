...
void insertBall()
{
  ...
  while (cur != nullptr)
  {
    if (posisi == counter)
    {
      ...
      { // apakah bola yang dimasukkan berada paling akhir
        createBall(peluru);
        newBall->prev = tail;
        tail->next = newBall;
        tail = newBall;
...
void removeBall()
{ // remove ball yang ditunjuk *del (*cur)
  del = cur;
  if (del == head)
  {
    head = head->next;
...
void game()
{
  ...
  while (true)
  {
    ...
    if (key == 'j')
    {
      ...
      if (numOfBalls < 7)
      { // buat bola bertambah ekstra ketika kurang dari 7
        createBall(getRandomNum());
        insertBallAtFirst();
      }
      ...
      { // mengisi semua petak dengan karakter '_'
        petak[i] = '_';
      }
